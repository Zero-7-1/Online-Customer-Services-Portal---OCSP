...
if(isset($_REQUEST['submitrequest'])){
 ...
 ($_REQUEST['requestdate'] == "")){
  ...
  $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert"> 
          All fileds are required </div>';
 } else {
   // Assigning User Values to Variable and Save to DB 
   ...
   $sql = "INSERT INTO submitrequest_tb(request_info, request_desc, requester_name, 
          ...
          requester_zip, requester_email, requester_mobile, request_date)
           VALUES 
          ('$rinfo','$rdesc', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', 
   ...
   if($conn->query($sql) == TRUE){
    ...
    $msg = '<div class="alert alert-success col-sm-6 ml-5 mt-2" role="alert"> 
            Request Submitted Successfully' . $genid .' </div>';
    session_start();
    $_SESSION['myid'] = $genid;
...
<!--  Start of Service Request Column -->
<!--  I want to do a major change on this like implementing 
 a predefined service offerrings for customer like mobile repair or pc repair etc.. -->
...
<div class="col-sm-9 col-md-10 mt-5">
  <form class="mx-5" action="" method="POST">
    ...
    <div class="form-group">
      ...
      placeholder="Enter your full name as per govt. ID" name="requestername">
    </div>
  
    <div class="form-group">
      <label for="inputRequestInfo">Request Info</label>
      <input type="text" class="form-control" id="inputRequestInfo" 
      placeholder="Choose a service request" name="requestinfo">
    </div>
  
    <div class="form-group">
      <label for="inputRequestDescription">Description</label>
      <input type="text" class="form-control" id="inputRequestDescription" 
      placeholder="Describe your issue briefly.." name="requestdesc">
    </div>
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        placeholder="House No. 123" name="requesteradd1">
      </div>
      <div class="form-group col-md-6">
...
<!-- Only Number for input fields for Zip, Mobile Number -->
<script>
  function isInputNumber(evt) {
    var ch = String.fromCharCode(evt.which);
    if (!(/[0-9]/.test(ch))) {  // JS regular expression 
      evt.preventDefault();
    }
  }
</script>
