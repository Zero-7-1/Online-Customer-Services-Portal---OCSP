<?php
  ...
  if(isset($_REQUEST['rSignup'])){
    // Checking for Empty Fields and making sure insert does not work for empty values 
    if(($_REQUEST['rName'] == "") || ($_REQUEST['rEmail'] == "") || 
    ($_REQUEST['rPassword'] == "")){
      $regmsg = '<div class="alert alert-warning mt-2" role="alert"> 
                 All Fields are Required </div>';
    } else {
      // checking for pre existing email 
      $sql = "SELECT r_email FROM requesterlogin_tb 
              WHERE r_email='".$_REQUEST['rEmail']."'";
      $result = $conn->query($sql);
      if($result->num_rows == 1){
        $regmsg = '<div class="alert alert-warning mt-2" role="alert"> 
                    This Email ID Already Registered </div>';
      } else {
        // Assigning User Values to Variable and this should not run with page refresh 
        // will show undefined index as we have written code first and it exceutes 
        // so we will use issset above so that this code runs only after user clicks 
        // sign up 
        ...
        if($conn->query($sql) == TRUE){
          $regmsg = '<div class="alert alert-success mt-2" role="alert"> 
                     Account Succefully Created </div>';
        } else {
